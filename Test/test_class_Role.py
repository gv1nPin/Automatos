import pytest
from Main.enums import RoleType, BenefitLevel
...
def test_benefit_level_federal():
    ...
    assert RoleType.COMBAT_VETERAN.get_benefit_level() == BenefitLevel.FEDERAL
    assert RoleType.DISABLED_VETERAN.get_benefit_level() == BenefitLevel.FEDERAL

def test_benefit_level_regional():
    """Проверка, что ветераны военной службы получают региональный бюджет."""
    assert RoleType.MILITARY_SERVICE_VETERAN.get_benefit_level() == BenefitLevel.REGIONAL

def test_benefit_level_none():
    """Проверка, что гражданские пациенты идут без льгот."""
    assert RoleType.NON_VETERAN.get_benefit_level() == BenefitLevel.NONE
...
def test_high_danger_level_allowance_disabled_veteran(danger_level):
    ...
    assert RoleType.DISABLED_VETERAN.get_max_daily_allowed(danger_level) == 10
...
def test_high_danger_level_allowance_combat_veteran(danger_level):
    ...
    assert RoleType.COMBAT_VETERAN.get_max_daily_allowed(danger_level) == 5

@pytest.mark.parametrize("role", [RoleType.MILITARY_SERVICE_VETERAN, RoleType.NON_VETERAN])
@pytest.mark.parametrize("danger_level", [3, 4, 5])
...
def test_docs_for_combat_and_disabled_veterans():
    ...
    for role in (RoleType.COMBAT_VETERAN, RoleType.DISABLED_VETERAN):
...
def test_docs_for_military_service_veteran():
    ...
    docs = RoleType.MILITARY_SERVICE_VETERAN.requires_additional_doc()
...
def test_docs_for_non_veteran():
    ...
    docs = RoleType.NON_VETERAN.requires_additional_doc()
    assert len(docs) == 2
    assert "Паспорт РФ" in docs
